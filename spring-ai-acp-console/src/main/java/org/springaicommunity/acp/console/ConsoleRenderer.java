 ...
 *
 * <p>
 * Separate from the loop so that styling and the line discipline live in one place:
 * assistant text arrives as deltas and must not be printed line by line, while tool
 * activity is whole lines that have to interrupt it cleanly. Styles go through JLine
 * rather than raw ANSI escapes, so a dumb terminal gets plain text instead of escape
 * codes.
 *
 * <p>
 * Public and not final: an application with its own idea of how a turn should look
 * registers a subclass as a bean. Every method is synchronized because a permission
 * question is asked from another thread, part-way through a streamed answer.
 */
public class ConsoleRenderer {
	...
	/**
	 * The latest accounting of the turn, shown once when it ends rather than every time
	 * it moves.
	 */
	...
	/**
	 * The blank line that separates one exchange from the next, written before each
	 * prompt.
	 */
