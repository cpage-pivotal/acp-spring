...
/**
 * Where the catalogue comes from, where downloads land, and what is allowed to reach the
 * network.
 *
...
public record RegistrySettings(URI url, Path cache, Duration refresh, boolean offline, boolean requireChecksum,
	...
	public RegistrySettings {
		...
		if (!scheme.equalsIgnoreCase("https") && !scheme.equalsIgnoreCase("file")) {
			// Plain http would mean fetching, over a network, the list of executables to
			// download
			// and the digests to check them against.
			throw new IllegalArgumentException("spring.acp.registry.url must be https or file but was '" + url + "'");
		}
	}

	/**
	 * Defaults: the published catalogue, a cache under the user's home, refreshed daily,
	 * checksums required.
	 *
	 * <p>
	 * {@code requireChecksum} defaults to on although it makes 9 of the registry's 19
	 * binary agents unreachable without a second property. That is the right way round:
	 * this feature downloads an executable and runs it with the application's credentials
	 * in its environment, and an agent whose publisher did not say what the bytes should
	 * be is a decision an operator should have to make in writing.
	 */
	 ...
	 *
	 * <p>
	 * Unlike {@code runtime-home}, which holds generated config a run can recreate for
	 * nothing, this holds hundreds of megabytes fetched over the network. A location that
	 * a reboot clears would mean re-downloading an agent on every machine restart.
	 */
	private static Path defaultCache() {
		...
		return base.resolve(".spring-ai-acp").resolve("agents").toAbsolutePath();
	}
