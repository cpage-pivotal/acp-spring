...
/**
 * Downloads an agent named by the registry, checks it is the bytes the registry said, and
 * unpacks it somewhere it can be run from.
 *
 * <p>
 * <strong>The digest is the point of this class, not a detail of it.</strong> Everything
 * else here is plumbing around one decision: this library is about to run a downloaded
 * executable in a process holding the application's model credentials, with the
 * application's workspace as its working directory. Verifying it against the digest the
 * registry publishes is the only thing standing between that and trusting a CDN and a
 * release host.
 *
 * <p>
 * Which is why {@code requireChecksum} defaults to on even though 9 of the registry's 19
 * binary agents publish no digest at all. Those nine are reachable, by an operator
 * writing {@code require-checksum: false} and meaning it. The failure names the agent and
 * says exactly that, because a refusal an operator cannot act on is just an outage.
 *
 * <p>
 * Installed agents are cached by id, version and digest, so a second application on the
 * same machine — or the same application after a restart — starts without a download. A
 * directory already holding the command is used as it is; nothing revalidates bytes it
...
public class AgentInstaller {
	...
	/**
	 * One install per artifact per JVM, so two threads starting an agent do not race on
	 * the files.
	 */
	...
	/**
	 * The executable for {@code artifact}, downloading and unpacking it if this machine
	 * has not got it already.
	 ...
	 *
	 * <p>
	 * The digest when there is one, so a republished archive under an unchanged version
	 * number is a different directory rather than a stale cache hit. The version alone
	 * when there is not, which is the cost of the entries that publish no digest.
	 */
	private String version(RegistryEntry entry, RegistryEntry.Artifact artifact) {
		return artifact.findSha256()
			.map(sha -> sha.substring(0, Math.min(16, sha.length())))
		...
		try {
			...
			if (sha256 != null && !sha256.equalsIgnoreCase(actual)) {
				throw new AgentInstallException("The download for '" + entry.id() + "' from " + artifact.archive()
						+ " hashes to " + actual + " but the registry says " + sha256 + "; refusing to run it");
			}
			if (sha256 == null) {
				logger.warn(
						"Installed '{}' from {} without verifying it: the registry publishes no sha256"
								+ " and require-checksum is off. Its sha256 is {}",
						entry.id(), artifact.archive(), actual);
			}
			...
			// Moved into place as one step, so a half-written install is never a usable
			// one.
	...
	/**
	 * Streams the download to disk, hashing as it goes. Returns the digest of what
	 * arrived.
	 */
	...
	/**
	 * What the archive actually held, for an entry whose {@code cmd} does not match its
	 * contents.
	 */
		...
		try (var paths = Files.walk(directory)) {
			paths.sorted(Comparator.reverseOrder()).forEach(path -> {
				try {
					...
					// Best effort: a leftover staging directory costs disk, not
					// correctness.
				}
			});
		}
