 ...
 *
 * <p>
 * goose — the reference runtime, and the one most likely to be installed this way by
 * someone evaluating the library — publishes {@code .tar.bz2}, which has no decompressor
 * in the JDK. That path hands off to the system {@code tar}, and this is the test that
...
class ArchivesTests {
	...
	void unpacksATarBz2ThroughTheSystemTar() throws Exception {
		...
		int exit = new ProcessBuilder("tar", "-cjf", archive.toString(), "-C", source.toString(), "agent").inheritIO()
			.start()
			.waitFor();
