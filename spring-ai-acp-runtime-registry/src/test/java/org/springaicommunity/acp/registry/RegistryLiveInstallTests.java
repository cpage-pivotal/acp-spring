 ...
 *
 * <p>
 * Opt-in rather than skip-when-absent, which is the one place this module departs from
 * how the rest of the suite gates its live tests. An agent that is not installed costs
 * nothing to check for; a download costs bandwidth on every build and fails on a machine
 * behind a proxy, and neither is a thing to inflict on someone running {@code mvn test}
 * to check an unrelated change.
 *
 * <pre>{@code
 * mvn -pl spring-ai-acp-runtime-registry test -Dspring-ai-acp.test.registry.live=true
 * }</pre>
 *
 * <p>
 * What it proves that the offline tests cannot: that the digests in the published
 * catalogue actually match the bytes on the release host. Everything else about this path
 * is exercised against archives built in the test; this is the one assertion that depends
 * on a third party having told the truth.
 */
@EnabledIfSystemProperty(named = "spring-ai-acp.test.registry.live", matches = "true")
class RegistryLiveInstallTests {

	/**
	 * Small, checksummed on every platform, and not an agent this library has an adapter
	 * for.
	 */
	...
	void installsARealAgentFromTheRealRegistry() throws Exception {
		...
		// The download, the digest check and the unpack all happen here: launch() is the
		// first
		// moment an agent is needed, which is why a misconfigured one does not stop a
		// context
		// refreshing.
	...
	void doesNotDownloadAnAgentItAlreadyHas() throws Exception {
		...
		RegistryEntry.Artifact artifact = ((RegistryEntry.Distribution.Binary) entry.distribution())
			.forPlatform(Platform.current())
			.orElseThrow();

		Path first = installer.install(entry, artifact);
		// A fresh installer, so the answer comes off the disk rather than out of the memo
		// table.
