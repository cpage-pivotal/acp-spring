 ...
 *
 * <p>
 * goose 1.51 opens an MCP connection by sending {@code server/discover} with
 * {@code MCP-Protocol-Version: 2026-07-28}, and falls back to plain {@code initialize}
 * when the server says it has never heard of the method. A server that predates the
 * version is supposed to say exactly that. The Tanzu MCP gateway instead answers with a
 * 400 whose JSON-RPC {@code id} is {@code "server-error"} rather than the request's, so
 * goose cannot correlate the error, never falls back, and the extension fails to start —
 * measured, with no message over ACP, stdout or stderr, whether the server is declared in
 * {@code session/new} or in goose's own config.
 *
 * <p>
 * So this says what the older server should have: a well-formed "method not found"
 * carrying the request's own id. goose then falls back and connects. And the version
 * header the gateway refuses outright is not passed upstream on anything else either.
 *
 * <p>
 * Opt-in ({@code spring.acp.runtimes.goose.mcp.answer-discover}) because it costs a
 * server that <em>does</em> implement discovery its newer protocol. Delete it the day
