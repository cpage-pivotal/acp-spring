...
class GooseRuntimeTests {
	...
	void launchesInAcpModeWithServerSideDefaults() {
		...
		assertThat(spec.env()).containsEntry("GOOSE_DISABLE_KEYRING", "1")
			.containsEntry("GOOSE_TELEMETRY_ENABLED", "false");
	}
	...
	void goosesOwnNameForAnOpenAiEndpointIsUsed() {
		// Goose reads OPENAI_HOST, not the OPENAI_BASE_URL the derivation rule would
		// produce, and it
	...
	void anEndpointPublishedUnderAPathKeepsThatPath() {
		// A gateway that gives every endpoint its own prefix, which is the shape that
		// made the
	...
	void theRouteIsLeftToGooseSoAModelThatWantsTheResponsesApiGetsIt() {
		// Measured: from one prefixed OPENAI_HOST, goose sent gpt-5.6-terra to
		// /v1/responses and
		// deepseek to /v1/chat/completions. Pinning OPENAI_BASE_PATH would take that
		// choice away and
	...
	void anApplicationThatMustPinTheDialectStillCan() {
		...
		AgentLaunchSpec.Stdio spec = (AgentLaunchSpec.Stdio) runtime.launch(settings().provider(provider)
			.runtimeOptions(Map.of("env.OPENAI_BASE_PATH", "v1/chat/completions"))
			.build());
	...
	void aVendorTheAgentAlreadyKnowsLeavesTheModelToTheWire() {
		// Nothing carried out of band, because the protocol can do it and report what
		// happened.
	...
	private AgentLaunchSpec.WebSocket served(Map<String, Object> serve) {
		AgentSettings settings = AgentSettings.builder("goose", workspace)
			.runtimeOptions(Map.of("serve", serve))
			.build();
	...
	/**
	 * The generated secret has to reach two places at once — the server's environment and
	 * this client's upgrade header — and the server refuses every connection if they
	 * disagree.
	 */
	@Test
	...
	void builtinsAndTheHardeningEnvironmentSurviveTheSwitchOfTransport() {
		AgentSettings settings = AgentSettings.builder("goose", workspace)
			.runtimeOptions(Map.of("serve", Map.of("transport", "websocket"), "builtins", "developer"))
			.build();
