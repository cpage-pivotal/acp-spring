...
class GooseContractTests extends AgentRuntimeContract {
	 ...
	 *
	 * <p>
	 * {@code gpt-5.6-luna} is the cheap end of the agentic coding line and still follows
	 * an instruction and calls a tool, which is all the contract asks of it;
	 * {@code gpt-5.4-mini} is the cheaper fallback if the first name retires. Goose
	 * advertises whatever its configured provider sells, so on a machine pointed at a
	 * different provider neither name matches and the base class falls back to the model
	 * Goose already has.
	 */
	@Override
