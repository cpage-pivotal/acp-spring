 ...
 *
 * <p>Small enough to be obvious and to avoid putting a Jackson version on an application's classpath
 * for the sake of one file written once at startup; the values come from a bound configuration
 * block, so they are already limited to the scalars, lists and maps YAML can express.
 */
final class Json {
	...
	private static void writeValue(StringBuilder out, Object value, int depth) {
		switch (value) {
			...
			default -> {
				...
				// A number or boolean that came through YAML as text should still land as one here.
