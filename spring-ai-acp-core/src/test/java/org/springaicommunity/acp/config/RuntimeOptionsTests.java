...
/**
 * Tier 3 arrives in whatever shape the binder produced, and an adapter must not care
 * which.
 */
class RuntimeOptionsTests {
	...
	void mergesFlatKeysThatShareAPrefix() {
		RuntimeOptions options = RuntimeOptions.of(new java.util.LinkedHashMap<>(Map.of("env.A", "1", "env.B", "2")));
