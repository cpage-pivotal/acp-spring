 ...
 *
 * <p>
 * Every test here asserts the same invariant from a different angle: a turn emits exactly
 * one terminal event and nothing after it. Consumers switch over {@code AgentEvent} with
 * no default branch, so a turn that ended silently would hang them, and a turn that ended
 * twice would emit after completion.
 */
class AgentTurnTests {
	...
	void streamsContentThenExactlyOneTerminalEvent() {
		...
		List<AgentEvent> events = AgentTurn.on(client, router)
	...
	void cancellingTheStreamCancelsTheTurnOnTheAgent() {
		...
		StepVerifier.create(AgentTurn.on(client, router).prompt(session, PROMPT, Duration.ofSeconds(30)))
			.thenAwait()
			.thenCancel()
	...
	void releasesTheSessionInTheRouterBeforeTheCallerCanStartTheNextTurn() {
		// FluxCreate runs the downstream's onComplete before the sink's onDispose, so a
		// caller
		// blocking on a turn is released while that turn's router registration could
		// still be in
		// place. The next turn then fails with "already has an active turn" — for a turn
		// that had
	...
	void reportsTheTurnAndItsToolCallsToTheObservations() {
		...
		assertThat(observations.events).containsExactly("started goose/s", "tool started t1 READ", "tool COMPLETED t1",
				"usage 4441/1050000", "completed END_TURN");
	}

	@Test
	void aTurnTheConsumerAbandonedIsStillReported() {
		// The subscriber is gone, so no terminal event reaches it — but something is
		// counting, and a
		...
		AgentTurn.on(client, router, observations)
			.prompt(session, PROMPT, Duration.ofSeconds(5), new AgentObservations.TurnContext("goose", "s", null, true))
			.subscribe()
			.dispose();
	...
	void observationsAreOptionalAndTheirAbsenceChangesNothing() {
		...
		List<AgentEvent> events = AgentTurn.on(client, router, null)
			.prompt(session, PROMPT, Duration.ofSeconds(5))
			.collectList()
			.block(Duration.ofSeconds(5));
	...
	/**
	 * Records the calls rather than the meters; what Micrometer does with them is its own
	 * test.
	 */
