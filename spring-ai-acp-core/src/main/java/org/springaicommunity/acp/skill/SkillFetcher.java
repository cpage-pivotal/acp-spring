...
/**
 * Gets a skill's files onto disk. The seam between {@link SkillInstaller}, which decides
 * what is safe to install, and where the files come from, which a test replaces with a
 * directory it writes.
 */
@FunctionalInterface
public interface SkillFetcher {
	 ...
	 * @param revision the commit that was checked out, or {@code null} for a source
	 * without one
	 */
	...
	/**
	 * Bundled skills from this application's classpath, Git skills with the {@code git}
	 * on the PATH.
	 */
