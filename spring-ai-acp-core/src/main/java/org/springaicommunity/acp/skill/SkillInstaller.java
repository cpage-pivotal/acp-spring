...
/**
 * Installs the configured skills into {@code <workspace>/.agents/skills/}, where goose,
 * opencode and codex all look for them: bundled ones copied out of the application's
 * classpath, Git ones fetched.
 *
 * <p>
 * The one thing this library writes into the workspace, and deliberately so. Everything
 * an adapter provisions goes under {@link AgentSettings#runtimeHome()}, because it is the
 * agent's own configuration; a skill is content the application asked the agent to have,
 * and the workspace is the only place every agent reads it from without being told.
 * Measured against goose 1.51.0, which has no setting that moves it.
 *
 * <p>
 * Each skill this installs carries a {@value #MARKER} file naming where it came from.
 * That is what lets a skill removed from the configuration be removed from the workspace,
 * and what stops this from overwriting a skill somebody put there by hand.
 *
 * <p>
 * Fails closed, as the buildpack did: a skill that cannot be fetched or does not pass the
 * checks stops the agent from starting, rather than starting it without a capability the
...
public final class SkillInstaller {
	...
	public static final String MARKER = ".spring-ai-acp-skill";

	private static final String SKILL_FILE = "SKILL.md";

	/**
	 * What this JVM has already installed. A pool starts one client per agent process and
	 * restarts them, and none of that should mean fetching the same commit again.
	 */
	...
	/**
	 * Installs {@code settings}' skills, once per workspace and list of skills in this
	 * JVM.
	 */
	...
	/**
	 * Makes {@code workspace}'s installed skills exactly {@code skills}: each one
	 * fetched, checked and copied in, and any this library installed earlier that is no
	 * longer configured removed.
	 */
	...
	private void install(Path directory, SkillSpec skill) {
		Path staging = null;
		try {
			staging = Files.createTempDirectory("spring-ai-acp-skill-");
			...
			if (Files.exists(target, LinkOption.NOFOLLOW_LINKS)) {
				if (!Files.isRegularFile(target.resolve(MARKER), LinkOption.NOFOLLOW_LINKS)) {
					throw new SkillInstallException("'" + target + "' exists and was not installed by spring-ai-acp; "
			...
			throw new SkillInstallException("Could not install skill " + skill.describe() + ": " + ex.getMessage(), ex);
		}
		finally {
			if (staging != null) {
	...
	/**
	 * The buildpack's checks: the path exists in the repository, it holds a
	 * {@code SKILL.md}, nothing in it is a symbolic link — a link could point anywhere on
	 * this machine, and the agent would read through it — and {@code SKILL.md} has the
	 * expected digest when one was given.
	 */
	private static void check(SkillSpec skill, Path checkout, Path source) throws IOException {
		// Real paths, not lexical ones: a directory above the skill that is itself a link
		// would pass a
	...
	/**
	 * Copies the skill without the repository metadata, keeping file modes so scripts
	 * stay executable.
	 */
