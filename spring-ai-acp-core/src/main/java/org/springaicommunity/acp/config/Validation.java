 ...
 *
 * <p>
 * These rules are carried over from the Goose wrapper this library succeeds, where they
 * were arrived at the hard way. They are strict on purpose: everything here ends up in a
 * subprocess environment, a config file on disk, or an outbound HTTP header.
 */
public final class Validation {
	...
	public static String requireName(String value, String what) {
		...
		if (!NAME.matcher(value).matches()) {
			throw new IllegalArgumentException(what + " must match " + NAME.pattern() + " but was '" + value + "'");
		}
		return value;
	}
	...
	public static String requireEnvName(String key) {
		...
		if (!ENV_NAME.matcher(key).matches()) {
			throw new IllegalArgumentException(
					"environment variable name must match " + ENV_NAME.pattern() + " but was '" + key + "'");
		}
		return key;
	}
	...
	/**
	 * Rejects CR and LF in a header value. A newline here is header injection, and the
	 * value is usually a credential, so the message names the header but never the value.
	 */
	...
	/**
	 * Bounds a credential and rejects line breaks in it. Both matter because the value is
	 * about to be written into a subprocess environment or a config file on disk, and a
	 * newline there can end the line early and change the meaning of what follows. The
	...
	/**
	 * Requires HTTPS, with two exceptions that a platform genuinely needs: loopback, and
	 * internal container-to-container routes. Userinfo is rejected outright — credentials
	 * belong in a header, not a URL that will be logged.
	 */
	public static URI requireSecureUrl(URI url, String what) {
		...
		throw new IllegalArgumentException(what + " must use https (plain http is allowed only for loopback and "
				+ INTERNAL_SUFFIX + ") but was '" + url + "'");
	}
