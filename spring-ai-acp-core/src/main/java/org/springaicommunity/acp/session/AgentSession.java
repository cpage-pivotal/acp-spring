...
/**
 * One ACP conversation: the caller's stable name, the agent's issued id, and the turn
 * permit that keeps them honest.
 *
 * <p>
 * The permit is a {@link Semaphore} rather than a lock because a turn is acquired on the
 * calling thread and released on whichever Reactor thread delivers the terminal event. A
...
public final class AgentSession {
	...
	/**
	 * Whether a caller acting for {@code candidate} (null for nobody) may use this
	 * session.
	 */
	...
	/**
	 * Lets go of what this session held beyond the agent: its MCP proxy routes, so the
	 * URLs the agent was given stop working. Called by the registry whenever it forgets
	 * the session, by whichever path; idempotent because there are several.
	 */
	void release() {
	...
	/**
	 * What the agent advertised when this session was created: the options it will
	 * accept, and the values it has for each. Read by {@code ConfigResolver} before it
	 * sets anything.
	 */
	...
	/**
	 * What the negotiated tier actually managed to apply. An application that asked for a
	 * model and got {@code on-unsupported: warn} can find out here which model it is
	 * really talking to.
	 */
	...
	/**
	 * The agent's current configuration, as last observed. Seeded from
	 * {@link #advertised()} and refreshed by every {@code session/set_config_option}
	 * response and {@code config_option_update}.
	 */
	 ...
	 * completion, error, timeout, cancellation — and releasing twice would hand out two
	 * permits.
	 */
	public void endTurn() {
		touch();
