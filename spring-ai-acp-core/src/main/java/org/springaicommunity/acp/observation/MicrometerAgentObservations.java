 ...
 *
 * <p>
 * The only class in this library that names Micrometer, which is what lets
 * {@code micrometer-observation} be an optional dependency: nothing reaches this unless
 * an application has both the jar and an {@link ObservationRegistry} to hand it.
 *
 * <p>
 * <strong>Started and stopped rather than observed.</strong> The idiomatic
 * {@code observation.observe(() -> work())} opens a thread-local scope around a block,
 * and a turn is not a block: it is subscribed on one thread and finished on whichever
 * transport thread delivers the agent's last frame, seconds or minutes later. Wrapping it
 * in a scope would close the observation when the subscription <em>call</em> returned,
 * timing the act of asking rather than the turn. So the observation is started on the
 * subscribing thread — where the caller's own observation is still current, which is how
 * the turn ends up a child of it — and stopped from wherever the turn actually ends.
 *
 * <p>
 * Tool call observations get their parent set explicitly for the mirror-image reason:
 * they are created on a transport thread that has no current observation, so there is
 * nothing for Micrometer to infer a parent from.
 *
 * <p>
 * A tool call the turn outlives is stopped anyway, tagged {@code unfinished}. An agent
 * that announces a call and never reports its end is not rare — a cancelled turn produces
 * one every time — and an observation left open is a leaked span and a timer that never
 * fires.
 */
public class MicrometerAgentObservations implements AgentObservations {

	/**
	 * Statuses after which no further update is expected, so the observation can be
	 * stopped.
	 */
	...
	/**
	 * @param turnConvention overrides the tags put on a turn, or {@code null} for the
	 * default
	...
	public TurnRecording turnStarted(TurnContext context) {
		...
		Observation observation = AcpObservationDocumentation.TURN
			.observation(this.turnConvention, DefaultAgentTurnObservationConvention.INSTANCE, () -> observationContext,
					this.registry)
			.start();
	...
	private final class MicrometerTurnRecording implements TurnRecording {
		...
		public void toolCallStarted(String toolCallId, String title, String kind) {
			...
			ToolCallObservationContext callContext = new ToolCallObservationContext(this.context.turn().runtimeId(),
					toolCallId, title, kind);
			// No current observation on this thread, so the parent is stated rather than
			// inferred.
			callContext.setParentObservation(this.turn);
			Observation observation = AcpObservationDocumentation.TOOL_CALL.observation(
					MicrometerAgentObservations.this.toolCallConvention, DefaultToolCallObservationConvention.INSTANCE,
					() -> callContext, MicrometerAgentObservations.this.registry);
			// An agent may repeat a tool call id; the first one keeps the observation,
			// and the
	...
	/**
	 * Carries a failed tool call's id onto its observation without an agent's message on
	 * it.
	 *
	 * <p>
	 * A tool failure's text comes from the agent and names paths, commands and
	 * occasionally the content of a file, none of which belongs on a span that leaves the
	 * process.
	 */
