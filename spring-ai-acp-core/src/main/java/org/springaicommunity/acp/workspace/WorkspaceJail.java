 ...
 *
 * <p>
 * {@code Path.normalize()} alone is not a jail. It removes {@code ..} textually, which
 * stops {@code workspace/../../etc/passwd} and stops nothing else: a symlink inside the
 * workspace pointing at {@code /etc} is an ordinary-looking relative path that resolves
 * outside, and an agent can create one with a single tool call before asking this client
 * to read through it. So every decision here is made on the <em>real</em> path —
 * {@link Path#toRealPath} with symlinks followed — and the root is realpathed too,
 * because on macOS a temp directory is reached through {@code /var}, which is itself a
 * link to {@code /private/var}, and a jail that compared the two spellings would reject
 * everything.
 *
 * <p>
 * For a path that does not exist yet, which is most writes, the walk goes up to the
 * deepest ancestor that does exist and realpaths that. What does not exist cannot be a
 * symlink, so the remaining segments are safe to append — and this is also why the check
 * must be repeated on every call rather than cached: the filesystem can change between
...
public final class WorkspaceJail {
	...
	/**
	 * Resolves a directory to run something in, which must already exist inside the
	 * workspace.
	 */
	...
	private Path confine(String requested) {
		...
		Path candidate;
		try {
			// A relative path is relative to the workspace, which is what the agent was
			// told its cwd is.
		...
		if (!realExisting.startsWith(root)) {
			throw new WorkspaceAccessException("'" + requested + "' resolves outside the workspace and was refused");
		}
		// Only the non-existent tail is re-attached, and a path that does not exist
		// cannot be a link.
	 ...
	 *
	 * <p>
	 * For a path inside the workspace that is the workspace itself at worst, since the
	 * jail's own root exists by construction. For one outside it, the walk finds whatever
	 * real directory the request was aimed at, and the caller's {@code startsWith} check
	 * then refuses it — which is why this method does not need to know where the boundary
	 * is.
	 */
