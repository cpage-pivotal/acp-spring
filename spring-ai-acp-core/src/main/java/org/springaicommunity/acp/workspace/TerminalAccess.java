 ...
 *
 * <p>
 * The most dangerous capability in the protocol, and the one whose defaults matter most:
 * a terminal is arbitrary code execution as the JVM's user, inside a process that has the
 * application's credentials on its environment. Off by default, and when it is on, the
 * commands run inside the workspace jail with their output bounded.
 *
 * <p>
 * {@code allowedCommands} is empty by default and empty means <em>all</em>, which reads
 * backwards until you notice that the capability itself is the gate: an application that
 * turned terminals on and then allowed nothing would have built something that cannot
 * work. The list is there for the case worth having — an application that wants the agent
 * to be able to run its test suite and nothing else. Matching is on the command name
 * alone, since the arguments are the agent's to choose.
 */
public record TerminalAccess(boolean enabled, Set<String> allowedCommands, long outputByteLimit,
	...
	public TerminalAccess {
		allowedCommands = allowedCommands == null ? Set.of()
				: allowedCommands.stream().map(c -> c.toLowerCase(Locale.ROOT)).collect(Collectors.toUnmodifiableSet());
	 ...
	 * {@code /usr/local/bin/mvn} and {@code mvn} are the same command — an allowlist that
	 * could be evaded by spelling out the path would not be one.
	 */
