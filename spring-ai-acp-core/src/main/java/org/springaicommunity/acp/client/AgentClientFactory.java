...
import org.springaicommunity.acp.event.SessionUpdateDecoder;
...
import org.springaicommunity.acp.process.AgentProcessSupervisor;
 ...
 *
 * <p>
 * The client capabilities declared here follow the settings, and default to lending the
 * agent nothing. This library runs server-side, where an agent asking to read a file or
 * open a terminal is asking a process with no human supervising it. What is not lent is
 * declared unsupported rather than advertised and then refused — an agent that knows it
 * cannot read files plans differently from one that discovers it mid-turn — and the
 * handler for a capability is registered only when that capability is on, so there is one
 * source of truth rather than two that can disagree.
 *
 * <p>
 * Nothing here names a runtime. Everything vendor-specific is behind
 * {@link AgentRuntime}: what to launch, what to write before launching, and what the
 * agent calls the options a client may set. Skills need no adapter: every runtime reads
 * them from the workspace, so they are installed here.
 */
public final class AgentClientFactory {

	private static final Logger logger = LoggerFactory.getLogger(AgentClientFactory.class);

	private static final String CLIENT_NAME = "spring-ai-acp";
	...
	public static AgentClient create(AgentRuntime runtime, AgentSettings settings, AgentObservations observations) {
		// Before the agent starts, because agents read their skills at startup, and here
		// rather than in
	...
	/**
	 * Starts watching the agent's own log, if this client started the agent and the
	 * adapter knows where the log is.
	 *
	 * <p>
	 * Both conditions matter. An adapter that reports no log directory is the normal case
	 * — every runtime but Goose today — and a client that attached to an agent somebody
	 * else launched cannot know what environment that process has, so it would be tailing
	 * a guess. The watcher exists at all because of what the protocol leaves out: an MCP
	 * server the agent could not load is reported nowhere on the wire, and on goose
	 * 1.51.0 nowhere on stdout or stderr either.
	 */
	 ...
	 *
	 * <p>
	 * The order is the point. A served agent that rejects its configuration exits about a
	 * second after it is spawned, so connecting first would produce "connection refused"
	 * for a problem the process already explained on its own output;
	 * {@link AgentProcessSupervisor} waits for readiness and reports what the agent said
	 * instead.
	 */
	private static AgentClient connectServed(AgentRuntime runtime, AgentSettings settings,
		...
		// stale
		// whatever the socket thinks, and the pool should replace the client rather than
		// reuse it.
		if (supervisor != null) {
			supervisor.addRestartListener(() -> connected.set(false));
		}

		Liveness liveness = new Liveness(true, () -> connected.get() && (supervisor == null || supervisor.isHealthy()));
		Runnable onClose = supervisor == null ? () -> {
		} : supervisor::close;

		// Only a server this library started has an environment it knows, and therefore a
		// log it can
	 ...
	 *
	 * <p>
	 * Split in two because "yes" and "I cannot tell" are different facts and a pool that
	 * confused them would either never replace a dead agent or replace a healthy one. A
	 * stdio connection genuinely cannot tell: {@code StdioAcpClientTransport} owns the
	 * child process in a private field, never reports a transport exception to the
	 * handler it accepts, and exposes no liveness of any kind, so the only evidence of a
	 ...
	 *
	 * <p>
	 * The seam between starting an agent and talking to one. A test drives a client over
	 * an in-memory transport through here, and it is where a runtime that attaches to
	 * something it did not spawn — an agent on a WebSocket, a sidecar — will come in.
	 */
	...
	/**
	 * Same, also watching a log that belongs to the agent on the other end of
	 * {@code launched}.
	 *
	 * <p>
	 * Separate from {@link #create} because there the rule can be enforced — this library
	 * started the process, so it knows the environment and therefore the path. Here the
	 * caller is asserting that the watcher belongs to this agent, which only a caller
	 * that knows both can do. The watcher is closed with the client.
	 */
	public static AgentClient connect(AgentRuntime runtime, AgentSettings settings, AcpClientTransport launched,
	...
	private static AgentClient connect(AgentRuntime runtime, AgentSettings settings, AcpClientTransport launched,
		...
		AcpClient.AsyncSpec spec = AcpClient.async(transport)
			...
			// Deliberately not sessionUpdateConsumer: see SessionUpdateDecoder for why a
			// raw handler.
		...
		if (settings.terminal().enabled()) {
			spec = spec.createTerminalHandler(terminals::create)
				.terminalOutputHandler(terminals::output)
				.waitForTerminalExitHandler(terminals::waitForExit)
				.killTerminalHandler(terminals::kill)
	...
	/**
	 * Signs the agent in with the method its adapter named, before anything asks it for a
	 * session.
	 *
	 * <p>
	 * Once per connection rather than per session, because that is what ACP scopes it to.
	 * A method the agent did not offer is skipped with a warning rather than sent: the
	 * agent may well be signed in some other way, and if it is not, its own
	 * "Authentication required" on {@code session/new} says so. A method it offered and
	 * then refused is a startup failure — the credentials are wrong, and every session
	...
	private static void authenticate(AcpAsyncClient acp, AgentRuntime runtime, AgentSettings settings,
		...
		try {
			...
			throw new AgentClientException("Runtime '" + runtime.id() + "' refused authentication with '" + method.get()
					+ "'" + diagnostics.settledSummary(), ex);
		}
	}
	 ...
	 *
	 * <p>
	 * Worth the class. When an agent refuses to start, the reason is on its stderr and
	 * nowhere else — "Configuration is invalid … Expected \"manual\" | \"auto\" |
	 * \"disabled\", got false", or "Authentication required" — while the protocol failure
	 * this library sees is a handshake that never completed. Logging stderr at debug and
	 * then reporting only the handshake leaves an operator with a message that names the
	 * runtime and nothing else, for a problem that is entirely explicable. Bounded
	 * because an agent that fails noisily should not put its whole log in an exception
	 * message.
	 */
	private static final class AgentDiagnostics {
		 ...
		 *
		 * <p>
		 * The SDK drains the child's stderr on its own scheduler, so whether a line
		 * written microseconds before the process died has been dispatched by the time
		 * the handshake gives up is a race — and one that resolves the wrong way often
		 * enough to matter, since the whole point is to have the explanation when things
		 * go wrong. Waiting costs nothing here: this path has already failed and is about
		 * to throw. An agent that said nothing costs the full half second, once, on the
		 * way to an error.
		 */
	...
	/**
	 * Answers a {@code session/request_permission}. An unanswered request stalls the turn
	 * forever, so this must produce an outcome for every request, including one the
	 * policy declines to choose for.
	 */
	private static Mono<AcpSchema.RequestPermissionResponse> handlePermission(AgentRuntime runtime,
		...
		return Mono.fromSupplier(() -> {
			...
			return new AcpSchema.RequestPermissionResponse(
					chosen.<AcpSchema.RequestPermissionOutcome>map(o -> new AcpSchema.PermissionSelected(o.optionId()))
						.orElseGet(AcpSchema.PermissionCancelled::new));
		})
			// A policy may be a person thinking it over, and the transport's inbound
			// thread is also the
			// one delivering the rest of the turn; blocking it would freeze the text they
			// are reading.
			.subscribeOn(Schedulers.boundedElastic());
	}

	/**
	 * What this client offers the agent, which is nothing unless the application said
	 * otherwise.
	 *
	 * <p>
	 * Declared identically on the builder and in the {@code initialize} request because
	 * the SDK uses the first to decide which inbound methods it will route and the second
	 * is what the agent reads; they are two halves of one statement and disagreeing would
	 * mean advertising something no handler answers.
	 */
