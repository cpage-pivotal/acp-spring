 ...
 *
 * <p>
 * The motivating case is an MCP server the agent could not connect to. ACP says nothing
 * about it — {@code session/new} succeeds, no {@code session/update} arrives, and the
 * application gets a session whose model quietly has no tools — while the agent writes a
 * perfectly clear sentence about it to a log file of its own. A notice is that sentence,
 * recovered.
 *
 * <p>
 * Deliberately three fields. Anything richer would be this library modelling one agent's
 * diagnostics, and an adapter that needed more would be a sign the protocol should carry
 * it.
 *
 * @param severity how the agent rated it
 * @param subject what it is about, spelled the way the application spelled it — an MCP
 * server name, so a notice can be matched against what was requested
...
public record AgentNotice(Severity severity, String subject, String detail) {
	...
	/**
	 * Whether this notice is about {@code name}, matched the way an application would
	 * spell it.
	 */
