 ...
 *
 * <p>
 * Tier one, and the only tier-one option whose default an application is asked never to
 * change lightly: {@code maxVersion} above {@link AcpProtocol#HIGHEST_SPOKEN} offers a
 * version this library cannot actually speak, which is useful for finding out what an
 * agent claims and for nothing else. See {@link AcpProtocol} for why that is a probe
...
public record ProtocolSettings(int maxVersion, boolean strict) {
	...
	/**
	 * Whether this offer reaches beyond what {@link AcpProtocol#HIGHEST_SPOKEN} can
	 * honor.
	 */
