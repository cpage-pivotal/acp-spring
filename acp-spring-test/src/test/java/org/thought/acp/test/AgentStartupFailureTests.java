 ...
 *
 * <p>This has a test because getting it wrong is cheap and costs someone an afternoon. An agent that
 * will not start explains itself on stderr — an invalid config file, a missing login — while the only
 * thing this library observes is a handshake that never completed. Reporting just the handshake gives
 * an operator a message that names the runtime and nothing else, for a problem the agent already
 * described in full.
 */
class AgentStartupFailureTests {
	...
	void theAgentsOwnExplanationIsInTheException() {
		assertThatThrownBy(() -> AgentClientFactory.create(new FailingRuntime("Configuration is invalid at line 3"),
				settings())).isInstanceOf(AgentClientException.class)
	...
	void theColourCodesAnAgentWritesForATerminalAreStrippedOut() {
		// OpenCode's real complaint arrives as "\u001B[91m\u001B[1mError: \u001B[0mConfiguration is
		...
		assertThatThrownBy(() -> AgentClientFactory
				...
				.hasMessageContaining("Error: bad config").hasMessageNotContaining("\u001B");
	}
	...
	void anAgentThatFailsSilentlyStillFailsCleanly() {
		assertThatThrownBy(() -> AgentClientFactory.create(new FailingRuntime(null), settings()))
				.isInstanceOf(AgentClientException.class).hasMessageContaining("Failed to initialize runtime");
	}
	...
	private record FailingRuntime(String complaint) implements AgentRuntime {
		 ...
		 *
		 * <p>The lingering is not padding. A process that dies within microseconds of writing to stderr
		 * races the SDK's own stderr plumbing, which subscribes to the child's error stream slightly
		 * after starting it and loses anything already written — measured at roughly one run in five.
		 * That race belongs to acp-core, and no real agent is in it: an agent that rejects its
		 * configuration has parsed a file first. Reproducing it here would only make this suite flaky
		 * about somebody else's bug.
		 */
		@Override
