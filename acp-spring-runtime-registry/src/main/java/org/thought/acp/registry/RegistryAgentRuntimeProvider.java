...
import org.thought.acp.runtime.AgentRuntimeProvider;

/**
 * Answers for any agent in the ACP registry that has no compiled adapter.
 *
 * <p>Nothing is downloaded here. Resolving an id reads the snapshot and builds a runtime; the agent
 * itself is fetched the first time that runtime is launched, which is the first time a prompt runs.
 * That keeps a misconfigured agent out of context refresh — the rule the whole library follows: an
 * application healthy apart from its agent stays up.
 */
