 ...
 *
 * <p>All offline: the bundled snapshot is on the classpath, which is why it is bundled.
 */
class AgentRegistryTests {
	...
	void readsAnNpxAgent() {
		...
		assertThat(gemini.distribution()).isInstanceOfSatisfying(RegistryEntry.Distribution.Npx.class, npx -> {
			...
			// The argument that makes it an ACP agent rather than a CLI, and the reason the registry
	...
	void readsAnEntryWithNoDigest() {
		// 9 of the registry's 19 binary agents are like this, which is why require-checksum exists
		...
		assertThat(cursor.distribution()).isInstanceOfSatisfying(RegistryEntry.Distribution.Binary.class,
				binary -> assertThat(binary.artifacts().values()).allSatisfy(
						artifact -> assertThat(artifact.findSha256()).isEmpty()));
	}
	...
	void skipsAnEntryItCannotUnderstand() {
		// The registry is a document 41 third parties contribute to. One of them shipping a shape
	...
	void fallsBackToTheBundledSnapshotWhenTheFetchFails() {
		// A CDN being briefly unreachable is not a reason for an application not to start. A stale
		// catalogue names older versions of the same agents, which is a much better answer.
		AgentRegistry registry = new AgentRegistry(new RegistrySettings(
				java.net.URI.create("https://cdn.agentclientprotocol.invalid/registry.json"), cache,
				java.time.Duration.ZERO, false, true, null));
	...
	void rejectsACatalogueUrlThatIsNotHttpsOrAFile() {
		...
		org.assertj.core.api.Assertions
				.assertThatThrownBy(() -> new RegistrySettings(java.net.URI.create("registry.json"), cache, null,
						false, true, null))
				.isInstanceOf(IllegalArgumentException.class).hasMessageContaining("https");
	}
}
