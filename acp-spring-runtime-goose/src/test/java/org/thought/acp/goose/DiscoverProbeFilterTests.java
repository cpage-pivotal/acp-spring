...
import org.thought.acp.mcp.McpRequestFilter;
import org.thought.acp.mcp.McpRequestFilter.McpRequest;
import org.thought.acp.mcp.McpRequestFilter.Outcome;
...
class DiscoverProbeFilterTests {
	...
	void answersTheProbeAsAServerThatHasNeverHeardOfItWouldWithTheRequestsOwnId() {
		Outcome numeric = filter.filter(post("{\"jsonrpc\":\"2.0\",\"id\":7,\"method\":\"server/discover\"}", Map.of()));
		Outcome text = filter.filter(post("{\"jsonrpc\":\"2.0\",\"id\":\"abc\",\"method\":\"server/discover\"}", Map.of()));
	...
	void isOffUnlessTheApplicationAsksForIt() {
		...
		assertThat(goose.mcpRequestFilters(settings.runtimeOptions(Map.of("mcp.answer-discover", "false")).build()))
			.isEmpty();
		assertThat(goose.mcpRequestFilters(settings.runtimeOptions(Map.of("mcp", Map.of("answer-discover", true)))
			.build())).singleElement().isInstanceOf(DiscoverProbeFilter.class);
	}
	...
	private static HttpResponse<String> send(URI url, String body, String version) throws Exception {
		return HttpClient.newHttpClient()
			.send(HttpRequest.newBuilder(url).timeout(LIMIT).header("Content-Type", "application/json")
				.header("MCP-Protocol-Version", version).POST(HttpRequest.BodyPublishers.ofString(body)).build(),
	...
	void withoutTheFilterGooseGetsAnAnswerItCannotCorrelate() throws Exception {
		// What goose 1.51 saw: an error whose id is not the request's, so it never falls back.
	...
	void withTheFilterTheProbeFailsCleanlyAndTheFallbackConnects() throws Exception {
		try (McpAccess access = new McpAccess(McpCredentialsProvider.none(), LIMIT, List.of(new DiscoverProbeFilter()))) {
	...
	/**
	 * The Tanzu MCP gateway's behaviour as measured: {@code server/discover}, or anything carrying
	 * the probe version, is refused with an error whose id is not the request's.
	 */
