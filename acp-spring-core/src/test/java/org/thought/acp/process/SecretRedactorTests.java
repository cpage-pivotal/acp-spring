...
/**
 * The shapes credentials actually appear in on an agent's output, taken from what the three
 * runtimes print when their log level is raised or their configuration is wrong.
 */
class SecretRedactorTests {
	...
	void redactsAssignments() {
		assertThat(SecretRedactor.redact("OPENAI_API_KEY=sk-live-abcdef1234"))
				.isEqualTo("OPENAI_API_KEY=[REDACTED]");
	}
	...
	void redactsJson() {
		assertThat(SecretRedactor.redact("{\"api_key\": \"sk-live-abcdef\", \"model\": \"gpt-5.4-mini\"}"))
				.contains("[REDACTED]").doesNotContain("sk-live-abcdef").contains("gpt-5.4-mini");
	}
