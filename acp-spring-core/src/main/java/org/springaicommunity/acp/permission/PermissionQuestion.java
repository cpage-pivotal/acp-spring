 ...
 *
 * <p>The tool name alone is enough for a policy but not for a person: {@code developer__shell} says
 * nothing about which command. The title is the agent's own one-line description of the call, which
 * is what a human actually needs to see before answering.
 *
...
public record PermissionQuestion(Optional<String> toolName, String title, AcpSchema.ToolKind kind,
	...
	/** The best short description available: the title, else the tool name, else a placeholder. */
