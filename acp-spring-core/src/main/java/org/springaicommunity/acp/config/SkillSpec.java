 ...
 *
 * <p>Portable, although ACP says nothing about skills. The Agent Skills layout — a directory per
 * skill under {@code .agents/skills/} in the agent's working directory — is read by goose, opencode
 * and codex alike, so installing one needs no adapter.
 *
 * <p>The rules are the ones the goose buildpack's {@code skills_configurator.sh} arrived at, with one
 * relaxation: a Git skill's {@code ref} is optional. See {@link Git}.
 */
public sealed interface SkillSpec {
	...
	/** A one-line, log-safe description of where this skill comes from. Never includes a token. */
	String describe();

	/**
	 * A skill packaged with the application, found on the classpath: inside the application's jar
	 * once it is built, under {@code target/classes} when it runs from an IDE. The default way to ship
	 * a skill, because it travels with the build that was tested with it.
	 *
	 ...
	 *
	 * <p>Without a {@code ref}, the repository's default branch is fetched, which means the skill can
	 * change between two starts of the same application; the installer says so. A full 40-character
	 * commit is the pinned form, and is checked against what Git actually resolved.
	 *
	 ...
	 * @param path the skill's directory inside the repository, or {@code null} for its root
	 * @param token a token for a private repository, sent as HTTP Basic credentials, or {@code null}
	 */
	record Git(String name, URI url, String ref, String path, String token, String sha256) implements SkillSpec {
		...
		/** A branch or tag Git will accept, and nothing an argument parser could read as an option. */
