...
final class SkillRules {
	...
	/** The buildpack's rule: a directory name every agent and every filesystem is happy with. */
	...
	static String name(String name, String source) {
		...
		if (!NAME.matcher(chosen).matches()) {
			throw new IllegalArgumentException(
					"skill name must match " + NAME.pattern() + " but was '" + chosen + "'");
		}
		return chosen;
	}
	...
	/**
	 * Relative, normalized and inside its root. The path is resolved against a checkout or a
	 * classpath, so {@code ..} or a leading slash would be a way out of it.
	 */
