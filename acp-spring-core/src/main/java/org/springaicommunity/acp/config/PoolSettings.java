 ...
 *
 * <p>An agent handles many sessions but runs one turn at a time per session and, in practice, is
 * bounded by its own model calls rather than by anything this library controls. So the reason to
 * run more than one process is concurrency across <em>unrelated</em> conversations, and the reason
 * not to is that each one is a full agent with its own memory footprint and its own provider
 * credentials on its environment. One is the right default for an application that serves requests
 * one at a time; anything more is a decision an operator makes with a load figure in hand.
 *
 * @param maxProcesses agent connections to keep, each backed by its own process
 * @param maxSessionsPerProcess named sessions one process may hold before another is preferred
 * @param maxRestarts restarts allowed inside a five-minute window before the runtime is given up on
 */
