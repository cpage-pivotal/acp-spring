 ...
 *
 * <p>Found through its {@code SKILL.md} rather than its directory, because a jar is not obliged to
 * have entries for directories. Two shapes of classpath are handled: a directory, which is what an
 * IDE or {@code mvn spring-boot:run} runs from, and a jar, which is what {@code java -jar} runs from.
 * Spring Boot's nested jars are reached through {@link JarURLConnection} like any other — Boot's own
 * connection extends it — so this needs nothing from Spring.
 */
public class BundledSkillFetcher {
	...
	/**
	 * Links are copied as links, not followed, so that the installer's check sees them and refuses the
	 * skill — as it would a link in a Git checkout.
	 */
	...
	/**
	 * The jar is not closed: with caching on, which is the default, it is shared with the class loader
	 * that is still running this application.
	 */
	private static void copyJarEntries(JarURLConnection connection, Path into) throws IOException {
		JarFile jar = connection.getJarFile();
		String prefix = connection.getEntryName().substring(0,
				connection.getEntryName().length() - SKILL_FILE.length());
	...
	/**
	 * A jar keeps no file modes, and a skill's instructions may run a script directly rather than
	 * through its interpreter. A file that starts with a shebang is meant to be run.
	 */
