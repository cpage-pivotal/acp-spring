 ...
 *
 * <p>Two shapes, and the difference between them is who owns the process. With {@link Stdio} the
 * transport owns it: the SDK spawns the child, writes to its stdin and reads its stdout, and the
 * process dies when the transport closes. With {@link WebSocket} the agent is a server, and this
 * library either supervises the server it started ({@link ManagedProcess}) or attaches to one
 * somebody else runs. Only Goose offers the second shape today; every other runtime is stdio-only,
 * which is why the sealed hierarchy exists rather than one record with optional fields.
 */
public sealed interface AgentLaunchSpec {
	...
	/**
	 * An agent reached over a WebSocket, optionally one this library starts and supervises.
	 *
	 * @param uri the ACP endpoint, e.g. {@code ws://127.0.0.1:3284/acp}
	 * @param headers sent on the upgrade request; where an auth secret goes, and never logged
	 * @param process the server to start first, or {@code null} to attach to a running one
	 */
	 ...
	 *
	 * @param healthUri polled until it answers 200, or {@code null} to treat "still alive" as ready
