 ...
 * files it reads before it starts, the names it happens to use for the options the protocol lets a
 * client set, and the handful of places its wire output carries vendor detail.
 *
 * <p>Note what is <em>not</em> here. There is no hook for "apply the model", because model selection
 * is a protocol operation and the core performs it identically for every agent; an adapter only says
 * what the option is <em>called</em>. Every method that could tempt an adapter into doing the core's
 * job has been left out on purpose — an adapter that grows one is a sign the core is missing
 * something.
 *
 * <p>An implementation that passes {@code AgentRuntimeContract} is swappable. That suite, not this
...
public interface AgentRuntime {
	 ...
	 *
	 * <p>Anything written belongs under {@link AgentSettings#runtimeHome()}, never in the workspace.
	 */
	 ...
	 *
	 * <p>Order is meaningful. Codex exposes two options in the mode family — {@code mode} for its
	 * approval policy and {@code collaboration_mode} for plan-versus-build — and the resolver walks
	 * the list looking for the one whose advertised values contain what was asked for.
	 */
	...
	/**
	 * The {@code category} values to fall back on when no id matches, most specific first.
	 *
	 * <p>Categories are the portable half of the option vocabulary and ids are the vendor half, so an
	 * agent nobody wrote an adapter for is still reachable through this. It is a fallback rather than
	 * the primary key because agents do not always set it: goose returns its {@code provider} option
	...
	/**
	 * Whether this runtime already carried {@code option} to the agent outside the protocol — an
	 * environment variable set in {@link #launch}, a line written by {@link #provision}.
	 *
	 * <p>Consulted only after every protocol path has failed, and only to decide whether the request
	 * counts as unsupported. It is a declaration, not an action: the work happened at launch, because
	 * that is the last moment an environment variable can still be set.
	 */
	...
	enum PortableOption {
		...
		/**
		 * The ACP {@code category} this option corresponds to, where the protocol names one. There is
		 * no standard category for a provider, which is why that one is empty.
		 */
