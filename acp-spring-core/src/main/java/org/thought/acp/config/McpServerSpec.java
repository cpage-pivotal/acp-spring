...
public sealed interface McpServerSpec {
	 ...
	 *
	 * <p>Worth having because a misconfigured MCP server is invisible from the client side: an agent
	 * that fails to connect to one still answers {@code session/new} normally and reports nothing, so
	 * the application gets a healthy-looking session whose model quietly has no tools. Naming what was
	 * requested does not detect that, but it puts "we asked for finops-mcp" in the log next to a model
	 * saying it has no finops tools. See "MCP servers fail silently" in {@code docs/design.md}.
	 *
	 * <p>Never includes headers or environment values: they carry the tokens.
	 */
	String describe();
	...
	record Http(String name, URI url, Map<String, String> headers) implements McpServerSpec {
		...
		/**
		 * The URL without anything that could be a credential: a token passed in the query string or as
		 * userinfo is as sensitive as the headers this deliberately never prints.
		 */
