 ...
 *
 * <p>Both fields are optional in ACP and at least one agent leaves them out, so this exists rather
 * than exposing {@code AcpSchema.Implementation} directly: a caller asking "which agent am I talking
 * to" gets a stable two-field answer instead of a schema record that may grow fields, and the
 * absent case is an empty {@link Optional} rather than a null inside a record.
 */
