...
import org.thought.acp.event.AgentEventMapper;
...
public final class AgentTurn {
	 ...
	 *
	 * <p>The context is built by the caller rather than derived here because the only parts of it
	 * worth recording — the runtime's id, the model the negotiated tier settled on — are things a
	 * turn does not know and its client does.
	 */
	public Flux<AgentEvent> prompt(AgentSession session, List<AcpSchema.ContentBlock> content, Duration timeout,
			AgentObservations.TurnContext context) {
		return Flux.create(sink -> start(sink, session, content, timeout, context),
	...
	private void start(FluxSink<AgentEvent> sink, AgentSession session, List<AcpSchema.ContentBlock> content,
		...
		AgentObservations.TurnRecording recording = observations.turnStarted(context == null
				? new AgentObservations.TurnContext("unknown", session.name(), null, false) : context);
		...
		try {
			...
			// Not unregistered here: the registration that exists belongs to another turn.
		...
		sink.onDispose(() -> {
			...
			if (cancelledByConsumer.get() && !finished.get()) {
				...
				// The consumer is gone, so no terminal event can be delivered to it — but something
				// is still counting, and a turn that vanished from the metrics would be the one
				// worth knowing about.
	 ...
	 *
	 * <p>The order of those two is load-bearing, and getting it wrong is a race that only shows up
	 * under a fast agent. {@code FluxCreate} runs the downstream's {@code onComplete} before it runs
	 * the sink's {@code onDispose} — so a caller blocking on this turn is released, and can start the
	 * next one, while the registration for this turn is still in the router. The next turn's
	 * {@code register} then finds the session occupied and fails with "already has an active turn",
	 * for a turn that had finished. Unregistering before the sink completes closes that window;
	 * {@code onDispose} still unregisters, for the cancellation path, and the second call is a no-op.
	 *
	 * <p>Nothing is lost by unregistering first: an update arriving after the turn's stop reason does
	 * not belong to this turn, and was already being dropped a microsecond later.
	 */
	private void finish(FluxSink<AgentEvent> sink, AtomicBoolean finished,
			AgentObservations.TurnRecording recording, String sessionId, AgentEvent terminal) {
		if (!finished.compareAndSet(false, true)) {
			return;
		}
		router.unregister(sessionId);
	...
	private static void record(AgentObservations.TurnRecording recording, AgentEvent event) {
		switch (event) {
			...
			case AgentEvent.ToolCallUpdated updated -> recording.toolCallUpdated(updated.id(),
					updated.status() == null ? null : updated.status().name());
			case AgentEvent.UsageUpdated usage -> recording.usage(usage.contextUsed(), usage.contextSize(),
	 ...
	 *
	 * <p>An exception message names a session id, a model or a path, and a tag that carries one
	 * turns a counter into a leak. The class name is the fact worth grouping by.
	 */
