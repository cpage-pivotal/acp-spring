...
/**
 * Turns a raw {@code session/update} notification into a typed one, skipping variants this SDK
 * version has no record for instead of failing on them.
 *
 * <p>This is why the client registers a raw notification handler rather than the SDK's
 * {@code sessionUpdateConsumer}. That consumer deserializes strictly: goose emits
 * {@code session_info_update} several times a turn, acp-core 0.17.0's {@code SessionUpdate}
 * hierarchy has no such subtype, and Jackson's failure to resolve the type id is logged at ERROR by
 * the SDK once per occurrence. The turn completes normally — the notification carries session
 * metadata a turn does not need — but an ERROR per turn for a benign case is worse than useless: it
 * teaches an operator to ignore that logger, which is where a real notification failure would
 * appear.
 *
 * <p>Suppressing the SDK's logger would have hidden the real failures too. Decoding leniently here
 * keeps them: anything that fails is reported, at a level that matches what it costs.
 */
public final class SessionUpdateDecoder {
	...
	/**
	 * @param params the notification's raw {@code params}, as the transport delivered them
