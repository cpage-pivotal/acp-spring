 ...
 *
 * <p>Two deliberate omissions. {@code user_message_chunk} is dropped: it echoes the prompt we just
 * sent, and replaying it to the caller as agent output is wrong. Updates carrying
 * {@code _meta.replay} are dropped for the same reason — {@code session/load} replays history
 * through the same channel a live turn uses, and history is not this turn's output.
 */
public final class AgentEventMapper {
	...
	/**
	 * @return the event this update represents, or empty when it carries nothing a caller needs
	 */
	...
	/**
	 * Extracts readable text from a content block. Images, audio and resource links carry no text
	 * and are skipped rather than rendered as a placeholder.
	 */
