...
class TomlTests {
	...
	void numbersAndBooleansThatArrivedAsTextStillLandAsNumbersAndBooleans() {
		// YAML quoting, or an environment variable, makes everything a string on the way
		// in.
	...
	void writesScalarsBeforeNestedTables() {
		// In TOML a key written after a [table] header belongs to that table, so order is
		// correctness,
		// not formatting.
		Map<String, Object> table = new LinkedHashMap<>();
	...
	void nestsTableHeadersByPath() {
		assertThat(Toml.write(Map.of("a", Map.of("b", Map.of("c", "d"))))).isEqualTo("\n[a]\n\n[a.b]\nc = \"d\"\n");
	}
