 ...
 *
 * <p>
 * The service-backed manager rather than the request-backed default, because the proxy
 * asks from its own threads, where there is no servlet request to hand one. It can
 * refresh a token but not obtain the first one: that takes the user's browser, which is
 * Spring Security's {@code oauth2Client()} flow on a request, and without a stored token
 * this manager answers {@code ClientAuthorizationRequiredException} — the same signal
 * that flow redirects on.
 *
 * <p>
 * Refreshes carry {@code resource=}, the RFC 8707 indicator the MCP authorization spec
 * requires on every token request. Measured against the Tanzu MCP gateway: tokens come
 * back with {@code aud} set to the server's URL, and a new refresh token each time.
 */
