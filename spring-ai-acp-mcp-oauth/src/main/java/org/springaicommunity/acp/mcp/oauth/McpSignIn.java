 ...
 *
 * <p>
 * Two answers, one per kind of application. A web application cannot sign anyone in from
 * inside a call: it throws, and Spring Security's redirect filter sends the user's
 * browser on the trip ({@link #redirect}). A terminal application has the user at the
 * keyboard and a browser on the same machine, so it runs the whole authorization-code
 * flow itself against a loopback redirect ({@code LoopbackSignIn}). The provider does not
 * care which; it asks for a redirect URI when it registers and for a sign-in when a token
 * is missing.
 */
public interface McpSignIn {
	 ...
	 * @throws StaleRegistration if the registration cannot be used and should be made
	 * again
	 */
	void signIn(McpServerSpec.Http server, SessionPrincipal principal);

	/**
	 * The registration this sign-in was handed cannot work — its loopback port is taken —
	 * and has been forgotten, so registering again and retrying once is the fix.
	 */
	 ...
	 * {@code {registrationId}} placeholders) and, for a missing token, throws Spring
