 ...
 *
 * <p>
 * Registers the beans Spring Security's {@code oauth2Client()} flow and mcp-security's
 * {@code McpClientOAuth2Configurer} look for — the registration repository, the dynamic
 * registration manager, the authorized-client service — plus the
 * {@link OAuth2McpCredentialsProvider} that puts those servers behind the loopback proxy,
 * the {@link McpSignIn} for a user without a token, and the resolver that says whose
 * session it is — the security context's user on the web, the operating-system user with
 * {@code mode: local}, where the sign-in is a browser opened on the spot and there is no
 * filter chain at all. The application still owns its {@code SecurityFilterChain}: it
 * signs users in however it likes and adds
 * {@code .with(McpClientOAuth2Configurer.mcpClientOAuth2(), mcp -> mcp.cimd(false))} so
 * that signing in to an MCP server works.
 *
 * <p>
 * Ordered before Boot's OAuth2 client auto-configuration, so the repository here is the
 * one Spring Security uses: dynamically registered clients have to be visible to the
 * redirect filter that sends users to sign in with them.
 */
@AutoConfiguration(
...
public class McpOAuthAutoConfiguration {
	...
	/**
	 * Also the application's {@code ClientRegistrationRepository}, which is what makes
	 * sign-in work. In memory unless {@link Jdbc} registered one first.
	 */
	@Bean
	@ConditionalOnMissingBean
	...
	/**
	 * Registrations and tokens in one private file: the default for a terminal
	 * application.
	 */
	@Bean
	@ConditionalOnMissingBean
	...
	FileMcpOAuthStore acpMcpOAuthFileStore(McpOAuthProperties properties, Environment environment) {
		FileMcpOAuthStore store = new FileMcpOAuthStore(
				properties.getFile() != null ? properties.getFile() : FileMcpOAuthStore
					.defaultLocation(environment.getProperty("spring.application.name", "spring-ai-acp")));
	...
	/**
	 * Where users' tokens live: written by Spring Security's sign-in flow, read by the
	 * proxy. In memory unless {@link Jdbc} registered one first.
	 */
	@Bean
	@ConditionalOnMissingBean
	...
	/**
	 * One refresh at a time per (server, user) in this JVM, unless {@link Jdbc} made it
	 * the database's.
	 */
	@Bean
	@ConditionalOnMissingBean
	...
	/**
	 * What happens for a user without a token: Spring Security's redirect on the web, a
	 * browser opened on the spot for a terminal application.
	 */
	@Bean
	@ConditionalOnMissingBean
	...
	OAuth2McpCredentialsProvider acpMcpOAuth2CredentialsProvider(AcpProperties acp, McpOAuthProperties properties,
		...
		String clientName = properties.getClientName() != null ? properties.getClientName()
				: environment.getProperty("spring.application.name", "spring-ai-acp");
	...
	/**
	 * The signed-in user of the request on the web; the operating-system user in a
	 * terminal.
	 */
	@Bean
	@ConditionalOnMissingBean
	...
	/**
	 * The database-backed store, shared by every instance. A member class, so it is only
	...
	static class Jdbc {
		...
		/**
		 * Every instance sharing the database takes the same row lock before refreshing,
		 * so a refresh token the authorization server honours once is spent once. Uses
		 * the application's transaction manager when there is one, else one over the
		 * {@code JdbcTemplate}'s own data source.
		 */
		@Bean
		@ConditionalOnMissingBean
		RefreshLock acpMcpRefreshLock(
