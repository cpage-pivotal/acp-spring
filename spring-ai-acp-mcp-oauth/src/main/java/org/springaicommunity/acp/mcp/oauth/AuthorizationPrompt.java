 ...
 *
 * <p>
 * A bean, so an application with its own idea of the console — a TUI, a web view — can
 * say it differently. The default prints the URL and opens it.
 */
@FunctionalInterface
public interface AuthorizationPrompt {
	 ...
	 *
	 * <p>
	 * Printed even when a browser opens, because a terminal that says only "waiting" is a
	 * dead end when the browser does not appear. And {@code open}/{@code xdg-open} are
	 * tried after {@link Desktop}, because a JVM started from a terminal on macOS reports
	 * no desktop at all.
	 */
