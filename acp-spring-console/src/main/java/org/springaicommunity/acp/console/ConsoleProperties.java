 ...
 *
 * <p>Everything here is presentation. What the agent is — runtime, model, MCP servers, skills — stays
 * under {@code spring.acp.*}, so the same configuration serves this console, a web front end and a
 * test.
 */
...
public class ConsoleProperties {
	...
	/**
	 * Whether the console runs. Unset: only when the application is attached to an interactive
	 * terminal, so a test, a CI job or a deployed service never waits on stdin. {@code true}: always,
	 * reading plain lines when there is no terminal. {@code false}: never.
	 */
	...
	/**
	 * The named session the conversation runs in, so the agent remembers earlier turns. {@code /new}
	...
	/**
	 * The history file in effect: the one configured, or one under the same config directory the MCP
	 * sign-ins use ({@code $XDG_CONFIG_HOME}, else {@code ~/.config}).
	 */
