 ...
 *
 * <p>For prototyping an agent — its {@code AGENTS.md}, its skills, its MCP servers — before there is
 * any other front end. Nothing here is registered unless the console will actually run (see
 * {@link ConsoleProperties#getEnabled()}), so the module is safe to leave on the classpath of an
 * application that is also tested and deployed.
 *
 * <p>Also registers the {@link PermissionPrompt} that {@code spring.acp.permissions.policy: ask}
 * needs, which puts each of the agent's permission requests to the person at the keyboard.
 */
...
public class ConsoleAutoConfiguration {
	...
	/**
	 * The system terminal, or a dumb one reading plain lines when the console was forced on without
	 * a terminal to run in.
	 */
	...
	ConsoleChat acpConsoleChat(AgentClient agent, Terminal terminal, ConsoleRenderer renderer,
		...
		String application = environment.getProperty("spring.application.name", "acp-spring");
	...
	/**
	 * Whether the JVM's stdin and stdout are an interactive terminal. On Java 21 a non-null
	 * {@link System#console()} already means that; from Java 22 there is always a console, and
	 * {@code isTerminal()} (reached reflectively, since this compiles for 21) is what says so.
	 */
	...
	/** {@code spring.acp.console.enabled}, with unset meaning "when there is a terminal to run in". */
