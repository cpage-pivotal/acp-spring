...
/** The loop against a stubbed agent and a scripted terminal: no agent process, no real tty. */
class ConsoleChatTests {
	...
	void streamsATurnInTheConfiguredSessionAndLeavesOnExit() {
		when(stream.events()).thenReturn(Flux.just(new AgentEvent.ToolCallStarted("1", "Read the cost report",
				AcpSchema.ToolKind.READ), new AgentEvent.Text("Spend is "), new AgentEvent.Text("up 4%."),
	...
	void aTurnThatFailsIsReportedAndTheConversationCarriesOn() {
		when(stream.events()).thenReturn(Flux.just(new AgentEvent.Failed(new IllegalStateException("agent went away"))))
			.thenReturn(Flux.just(new AgentEvent.Text("back"), new AgentEvent.Completed(AcpSchema.StopReason.END_TURN)));
