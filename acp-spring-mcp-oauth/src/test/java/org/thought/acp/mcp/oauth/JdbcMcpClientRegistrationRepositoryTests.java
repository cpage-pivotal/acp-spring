...
class JdbcMcpClientRegistrationRepositoryTests {
	...
	void springSecuritysTokenStoreWorksOnTopOfIt() {
		...
		tokens.saveAuthorizedClient(new OAuth2AuthorizedClient(repository.findByRegistrationId("finops-mcp"), "alice",
				new OAuth2AccessToken(OAuth2AccessToken.TokenType.BEARER, "access-1", now, now.plusSeconds(3600),
						Set.of("openid")),
				new OAuth2RefreshToken("refresh-1", now)), new TestingAuthenticationToken("alice", null));

		OAuth2AuthorizedClient loaded = new JdbcOAuth2AuthorizedClientService(jdbc,
				new JdbcMcpClientRegistrationRepository(jdbc)).loadAuthorizedClient("finops-mcp", "alice");
