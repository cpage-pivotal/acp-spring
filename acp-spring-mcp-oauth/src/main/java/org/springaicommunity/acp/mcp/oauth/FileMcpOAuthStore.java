...
/**
 * Client registrations and tokens in one file, for a terminal application run by one person.
 *
 * <p>Under the user's config directory ({@code $XDG_CONFIG_HOME}, else {@code ~/.config}) rather
 * than the project, and mode 600: these are this person's credentials for a shared gateway, and a
 * token that lands in the working directory eventually lands in a commit. Written whole, to a
 * temporary file moved into place, so a crash mid-write leaves the previous version rather than half
 * of one. A file that cannot be read is treated as empty — a reason to sign in again, not to fail.
 *
 * <p>Registrations are kept for the same reason tokens are: dynamic registration is a write on the
 * authorization server, and registering on every start leaves a trail of dead client ids.
 */
public final class FileMcpOAuthStore {
	...
	public static Path defaultLocation(String application) {
		...
		Path config = xdg == null || xdg.isBlank() ? Paths.get(System.getProperty("user.home"), ".config") : Paths.get(xdg);
	...
	/** The registration half, which is also the application's {@code ClientRegistrationRepository}. */
	...
	public final class Registrations implements McpClientRegistrationRepository {
		...
		public void addClientRegistration(ClientRegistration registration, String resourceId) {
			update(root -> root.withObjectProperty("registrations").set(registration.getRegistrationId(), write(registration, resourceId)));
		}
		...
		/**
		 * Forgets a registration, and every token issued under it — a token is only refreshable by the
	...
	private final class Tokens implements OAuth2AuthorizedClientService {
		...
		public void saveAuthorizedClient(OAuth2AuthorizedClient client, Authentication principal) {
			...
			update(root -> root.withObjectProperty("tokens").withObjectProperty(client.getClientRegistration().getRegistrationId())
	...
	private static void ownerOnly(Path path) {
		try {
			...
			// Not a POSIX filesystem: the token is no less valid, the file just less private.
		}
	}

	/** Exposed for tests: the permissions the file ended up with, where the platform has them. */
	...
	private static ObjectNode write(ClientRegistration registration, String resourceId) {
		...
		node.put("client_secret", StringUtils.hasText(registration.getClientSecret()) ? registration.getClientSecret() : null);
