 ...
 *
 * <p>Needed once per MCP server, when registering with its authorization server: the redirect URI
 * sent then has to be one the user's browser can reach, and the request that triggered the
 * registration is the best evidence of what that is. Behind a proxy that means the application must
 * honour {@code X-Forwarded-*} ({@code server.forward-headers-strategy}); where it cannot know, set
