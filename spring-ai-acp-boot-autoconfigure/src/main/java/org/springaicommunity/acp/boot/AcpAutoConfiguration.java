 ...
 *
 * <p>
 * Registering a runtime means contributing an {@link AgentRuntime} bean; the one whose
 * {@code id()} matches {@code spring.acp.runtime} is selected. Each first-party adapter
 * registers itself when it is on the classpath, so an application chooses its agents by
 * dependency and then picks one of them by property — and an application that wants its
 * own adapter for an agent this library ships just declares the bean, because every
 * registration backs off by name.
 */
@AutoConfiguration
...
public class AcpAutoConfiguration {
	 ...
	 *
	 * <p>
	 * Not a style choice. Boot reads the condition on a configuration class from the
	 * bytecode, without loading the class it names; on a {@code @Bean} method it has to
	 * reflect over the annotation, which throws {@code TypeNotPresentException} for an
	 * absent adapter — and then logs the failure and registers the bean anyway, so the
	 * application dies on {@code NoClassDefFoundError} at refresh. Since the whole point
	 * of these being optional dependencies is that an application ships only the agents
	 * it wants, the condition has to hold when the class is genuinely missing.
	 */
	 ...
	 *
	 * <p>
	 * A {@code McpCredentialsProvider} bean puts the HTTP MCP servers it answers for
	 * behind the loopback proxy, with that session's credentials; a
	 * {@code SessionPrincipalResolver} bean says whose session it is when a prompt does
	 * not. Neither is required, and without them every server is handed to the agent as
	 * configured. A {@code PermissionPrompt} bean is who {@code permissions.policy: ask}
	 * asks, and that policy fails to start without one.
	 */
	@Bean
	@ConditionalOnMissingBean
	AgentSettings acpAgentSettings(AcpProperties properties, SelectedRuntime selected,
		...
		return AgentSettings.builder(runtime, workspace)
			...
			.mcp(properties.toMcpSettings()
				.withCredentials(credentials.getIfAvailable())
	 ...
	 *
	 * <p>
	 * Always pooled, even at the default of one process, because two of the pool's three
	 * jobs apply to a single connection as much as to four: sweeping sessions nobody has
	 * touched since the TTL, and replacing a connection whose agent has gone. The third —
	 * running unrelated conversations at once — is the one that needs
	 * {@code spring.acp.pool.max-processes}.
	 *
	 * <p>
	 * Connections are opened on first use rather than at refresh, so an application whose
	 * agent is misconfigured starts and reports it on the first prompt rather than
	 * failing to start. That is the wrapper's rule carried over: an application healthy
	 * apart from its agent stays up.
	 */
	...
	/**
	 * The {@code GooseExecutor}-shaped facade, for applications migrating off the
	 * buildpack's wrapper. Costs nothing when unused, and having it registered is what
	 * makes the migration a change of import rather than a change of wiring.
	 */
	@Bean
	@ConditionalOnMissingBean
