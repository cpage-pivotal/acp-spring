 ...
 *
 * <p>Useful because it puts an agent where a Spring AI application already has a seam — advisors,
 * {@code ChatClient}, evaluators, the observation stack — without that application learning a second
 * programming model. It is a real adapter rather than a wrapper, because the two models disagree
 * about one important thing, and pretending otherwise would produce an object that looks right and
 * costs twice as much per call.
 *
 * <h2>The disagreement: who owns the conversation</h2>
 *
 * <p>A chat completion is stateless. Spring AI sends the whole history every time, which is why
 * {@code ChatMemory} exists and why a {@code Prompt} carries a list of messages. An ACP session is
 * stateful: the conversation lives inside the agent process, which is also holding a file tree, a
 * plan and a set of tool results that no message list can carry.
 *
 * <p>So this adapter reads {@link AcpChatOptions#getSession()} and behaves differently:
 *
 * <ul>
 * <li><strong>No session named.</strong> Every call is a throwaway ACP session that knows nothing,
 * so the whole prompt is sent, rendered with role labels. This is the closest thing to chat
 * completion semantics and the right default for an application using {@code ChatMemory} to manage
 * history itself.</li>
 * <li><strong>A session named.</strong> The agent already has everything up to its own last reply,
 * so only the messages <em>after</em> the last assistant message are sent. Sending the history again
 * would make the agent read its own previous answers as new instructions, and would bill for the
 * whole conversation on every turn.</li>
 * </ul>
 *
 * <p>Combining a named session with a {@code ChatMemory} advisor therefore means two memories of the
 * same conversation, and the agent's is the one that matters. Pick one.
 *
 * <h2>Options that do not exist over here</h2>
 *
 * <p>{@code temperature}, {@code topP}, {@code topK}, {@code maxTokens}, {@code stopSequences} and
 * the penalties have no ACP equivalent: the agent owns the inference call and the protocol gives a
 * client no way to reach into it. They are logged once, by name, rather than dropped quietly — an
 * application that set {@code temperature: 0} for determinism deserves to be told it did not get it.
 *
 * <p>Tool calling is the same story from the other side. The agent has its own tools and runs them
 * itself; {@code ToolCallbacks} registered on the Spring AI side are not visible to it. Tool
 * activity surfaces as {@code ToolCallStarted} and {@code ToolCallUpdated} on the native
 * {@link AgentClient} API, which is where an application that cares about it should be.
 */
public class AcpChatModel implements ChatModel {
		...
		return Flux.defer(() -> {
			...
			return spec(prompt, options).stream().events().handle((event, sink) -> {
				switch (event) {
					...
					default -> {
						// Thoughts, tool calls and plans are agent activity, not assistant output.
						// They are on the AgentClient API, which is where an application that wants
	 ...
	 *
	 * <p>Where the two models are reconciled. See the class documentation for the argument; this is
	 * the three lines that implement it.
	 */
	private String render(Prompt prompt, AcpChatOptions options) {
		...
		return toSend.stream().map(AcpChatModel::render).filter(text -> !text.isBlank())
				.reduce((a, b) -> a + "\n\n" + b).orElse("");
	}
	 ...
	 *
	 * <p>The agent's own last reply is the watermark, because it is the last thing both sides agree
	 * happened. Anything before it is history the agent is holding; anything after it is new.
	 */
	 ...
	 * One message as text.
	 *
	 * <p>A user message goes over bare, because that is what the agent expects to be prompted with.
	 * Everything else is labelled, because an agent reading a transcript needs to know which lines
	 * were its own — and ACP's prompt is content blocks with no roles to put them in.
	 */
	...
	private AcpChatOptions options(Prompt prompt) {
		...
		return AcpChatOptions.builder().model(requested.getModel() == null ? defaultOptions.getModel()
				: requested.getModel()).session(defaultOptions.getSession()).mode(defaultOptions.getMode())
				.timeout(defaultOptions.getTimeout()).build();
	}
	...
	private void reportUnsupported(ChatOptions requested) {
		...
		ignored.stream().filter(reportedUnsupported::add)
	 ...
	 *
	 * <p>{@code END_TURN} becomes {@code STOP} and {@code MAX_TOKENS} becomes {@code LENGTH} because
	 * those are what an advisor or an evaluator written against OpenAI is looking for. The two ACP
	 * reasons with no equivalent keep their own names rather than being flattened into one of these.
	 */
	...
	/**
	 * Response metadata, including ACP usage — as key values rather than as a {@code Usage}.
	 *
	 * <p>Spring AI's {@code Usage} is prompt tokens and completion tokens. ACP's {@code usage_update}
	 * is context window consumed out of context window size, cumulative for the session, plus an
	 * optional cost. Those are different measurements, and putting one in the other's field would put
	 * a number that means "the whole conversation so far" where every dashboard reads "this call".
	 */
	private ChatResponseMetadata metadata(AcpChatOptions options, AgentEvent.UsageUpdated usage) {
		ChatResponseMetadata.Builder builder = ChatResponseMetadata.builder()
				.model(modelOf(options)).keyValue("acp.runtime", client.runtimeId());
		if (options.getSession() != null) {
			builder = builder.keyValue("acp.session", options.getSession());
		}
		if (usage != null) {
			builder = builder.keyValue("acp.context.used", usage.contextUsed()).keyValue("acp.context.size",
					usage.contextSize());
			if (usage.costAmount() != null) {
				builder = builder.keyValue("acp.cost.amount", usage.costAmount()).keyValue("acp.cost.currency",
						usage.costCurrency() == null ? "" : usage.costCurrency());
			}
		}
		return builder.build();
	}

	/** The model the session really has, where the negotiated tier knows it; else what was asked. */
		...
		if (options.getSession() != null) {
			String applied = client.session(options.getSession())
					.flatMap(session -> session.configuration()
							.applied(org.thought.acp.runtime.AgentRuntime.PortableOption.MODEL))
					.orElse(null);
			if (applied != null) {
				return applied;
			}
